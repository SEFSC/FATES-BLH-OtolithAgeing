...
subtitle: "Using the `multimodal_configs.yml` file"
metadata-files:
  - ../../_metadata.yml
order: 3
---

A `multimodal_configs.yml` file is an easy way to control model performance. The most important settings in this file to pay attention to are:
...
```{.yaml filename="multimodal_configs.yml"}
# --------------------------------------------------------------------------------------------
...
Storing the `multimodal_configs.yml` file in the same directory as the model scripts is advantageous when [running the model](usage.qmd#running-the-model) because you will not need to include the full directory path when you specify which configuration file to use. Since you will be executing the model script from the directory that script resides, the system will automatically find the `multimodal_configs.yml` file in that same directory.
